import {
	ClientRegistration,
	DeviceAuth,
	GetCredentialsOptions,
	SSOAccount,
	SSOAuthEvent,
	SSOAuthEventListener,
	SSOAuthOptions,
	SSOCredentials,
	SSORole,
	StartAuthOptions,
	TokenData,
} from "./types.js"

const DEFAULT_PROXY_URL = process.env.AWS_STATIC_SSO_PROXY_URL ?? ""

const STORAGE_KEYS = {
	client: (region: string) => `aws-sso:client:${region}`,
	token: (region: string) => `aws-sso:token:${region}`,
	startUrl: "aws-sso:startUrl",
	region: "aws-sso:region",
} as const

export class SSOError extends Error {
	constructor(
		message: string,
		public status: number,
		public body?: string,
	) {
		super(message)
		this.name = "SSOError"
	}
}

export class SSOAuth {
	private startUrl: string
	private region: string
	private proxyUrl: string
	private listeners: SSOAuthEventListener[] = []

	constructor(options: SSOAuthOptions) {
		this.startUrl = options.startUrl
		this.region = options.region
		const proxyUrl = options.proxyUrl || DEFAULT_PROXY_URL
		if (!proxyUrl) throw new Error("proxyUrl is required (pass it in options or set AWS_STATIC_SSO_PROXY_URL)")
		this.proxyUrl = proxyUrl.replace(/\/$/, "")
	}

	on(listener: SSOAuthEventListener): void {
		this.listeners.push(listener)
	}

	off(listener: SSOAuthEventListener): void {
		this.listeners = this.listeners.filter(l => l !== listener)
	}

	private emit(event: SSOAuthEvent): void {
		for (const listener of this.listeners) {
			listener(event)
		}
	}

	private async request(host: string, path: string, body?: unknown): Promise<Response> {
		const url = `${this.proxyUrl}/${host}${path}`
		const response = await fetch(url, {
			method: body ? "POST" : "GET",
			headers: body ? { "Content-Type": "application/json" } : undefined,
			body: body ? JSON.stringify(body) : undefined,
		})
		if (!response.ok) {
			const text = await response.text()
			throw new SSOError(`AWS SSO request failed: ${response.status}`, response.status, text)
		}
		return response
	}

	private oidcHost(): string {
		return `oidc.${this.region}.amazonaws.com`
	}

	private portalHost(): string {
		return `portal.sso.${this.region}.amazonaws.com`
	}

	private loadClient(): ClientRegistration | null {
		const raw = localStorage.getItem(STORAGE_KEYS.client(this.region))
		if (!raw) return null
		const client: ClientRegistration = JSON.parse(raw)
		if (Date.now() >= client.expiresAt) {
			localStorage.removeItem(STORAGE_KEYS.client(this.region))
			return null
		}
		return client
	}

	private saveClient(client: ClientRegistration): void {
		localStorage.setItem(STORAGE_KEYS.client(this.region), JSON.stringify(client))
	}

	private loadToken(): TokenData | null {
		const raw = localStorage.getItem(STORAGE_KEYS.token(this.region))
		if (!raw) return null
		const token: TokenData = JSON.parse(raw)
		if (Date.now() >= token.expiresAt) {
			localStorage.removeItem(STORAGE_KEYS.token(this.region))
			this.emit({ type: "tokenExpired" })
			return null
		}
		return token
	}

	private saveToken(token: TokenData): void {
		localStorage.setItem(STORAGE_KEYS.token(this.region), JSON.stringify(token))
		localStorage.setItem(STORAGE_KEYS.startUrl, this.startUrl)
		localStorage.setItem(STORAGE_KEYS.region, this.region)
	}

	private async registerClient(): Promise<ClientRegistration> {
		const existing = this.loadClient()
		if (existing) return existing

		const response = await this.request(this.oidcHost(), "/client/register", {
			clientName: "aws-static-sso",
			clientType: "public",
		})
		const data = await response.json() as {
			clientId: string
			clientSecret: string
			clientSecretExpiresAt: number
		}
		const client: ClientRegistration = {
			clientId: data.clientId,
			clientSecret: data.clientSecret,
			expiresAt: data.clientSecretExpiresAt * 1000,
		}
		this.saveClient(client)
		return client
	}

	async startAuth(options?: StartAuthOptions): Promise<DeviceAuth> {
		const client = await this.registerClient()
		const response = await this.request(this.oidcHost(), "/device_authorization", {
			clientId: client.clientId,
			clientSecret: client.clientSecret,
			startUrl: this.startUrl,
		})
		const data = await response.json() as {
			deviceCode: string
			userCode: string
			verificationUri: string
			verificationUriComplete: string
			expiresIn: number
			interval: number
		}
		const deviceAuth: DeviceAuth = {
			deviceCode: data.deviceCode,
			userCode: data.userCode,
			verificationUri: data.verificationUri,
			verificationUriComplete: data.verificationUriComplete,
			expiresIn: data.expiresIn,
			interval: data.interval,
		}
		this.emit({ type: "deviceCodeReady", deviceAuth })
		return deviceAuth
	}

	async pollForToken(deviceAuth: DeviceAuth, signal?: AbortSignal): Promise<TokenData> {
		const client = this.loadClient()
		if (!client) throw new Error("No registered client; call startAuth() first")

		let interval = deviceAuth.interval * 1000
		const deadline = Date.now() + deviceAuth.expiresIn * 1000

		while (Date.now() < deadline) {
			if (signal?.aborted) throw new DOMException("Aborted", "AbortError")

			await new Promise<void>((resolve, reject) => {
				const timer = setTimeout(resolve, interval)
				signal?.addEventListener("abort", () => {
					clearTimeout(timer)
					reject(new DOMException("Aborted", "AbortError"))
				}, { once: true })
			})

			try {
				const response = await this.request(this.oidcHost(), "/token", {
					clientId: client.clientId,
					clientSecret: client.clientSecret,
					deviceCode: deviceAuth.deviceCode,
					grantType: "urn:ietf:params:oauth:grant-type:device_code",
				})
				const data = await response.json() as {
					accessToken: string
					expiresIn: number
					refreshToken?: string
				}
				const token: TokenData = {
					accessToken: data.accessToken,
					expiresAt: Date.now() + data.expiresIn * 1000,
					refreshToken: data.refreshToken,
				}
				this.saveToken(token)
				this.emit({ type: "tokenAcquired", token })
				return token
			} catch (err) {
				if (err instanceof SSOError && err.body) {
					let parsed: { error?: string }
					try { parsed = JSON.parse(err.body) } catch { throw err }

					if (parsed.error === "authorization_pending") continue
					if (parsed.error === "slow_down") {
						interval += 5000
						continue
					}
					if (parsed.error === "expired_token") {
						throw new Error("Device code expired; restart auth flow")
					}
				}
				throw err
			}
		}

		throw new Error("Device code expired; restart auth flow")
	}

	getSession(): SSOSession | null {
		const token = this.loadToken()
		if (!token) return null
		return new SSOSession(token, this.proxyUrl, this.region, this)
	}

	clearStorage(): void {
		localStorage.removeItem(STORAGE_KEYS.client(this.region))
		localStorage.removeItem(STORAGE_KEYS.token(this.region))
		localStorage.removeItem(STORAGE_KEYS.startUrl)
		localStorage.removeItem(STORAGE_KEYS.region)
	}
}

export class SSOSession {
	private refreshTimer: ReturnType<typeof setTimeout> | null = null

	constructor(
		private token: TokenData,
		private proxyUrl: string,
		private region: string,
		private auth: SSOAuth,
	) {
		this.scheduleRefresh()
	}

	private scheduleRefresh(): void {
		const msUntilExpiry = this.token.expiresAt - Date.now()
		// Refresh 5 minutes before expiry
		const refreshIn = Math.max(0, msUntilExpiry - 5 * 60 * 1000)
		this.refreshTimer = setTimeout(() => {
			this.token = null!
			this.auth["emit"]({ type: "tokenExpired" })
		}, refreshIn)
	}

	private portalHost(): string {
		return `portal.sso.${this.region}.amazonaws.com`
	}

	private async request(path: string, headers?: Record<string, string>): Promise<Response> {
		const url = `${this.proxyUrl}/${this.portalHost()}${path}`
		const response = await fetch(url, {
			headers: {
				"x-amz-sso_bearer_token": this.token.accessToken,
				...headers,
			},
		})
		if (!response.ok) {
			const text = await response.text()
			throw new SSOError(`AWS SSO request failed: ${response.status}`, response.status, text)
		}
		return response
	}

	async listAccounts(): Promise<SSOAccount[]> {
		const response = await this.request("/assignment/accounts")
		const data = await response.json() as { accountList: Array<{
			accountId: string
			accountName: string
			emailAddress: string
		}> }
		return data.accountList.map(a => ({
			accountId: a.accountId,
			accountName: a.accountName,
			emailAddress: a.emailAddress,
		}))
	}

	async listAccountRoles(accountId: string): Promise<SSORole[]> {
		const response = await this.request(
			`/assignment/roles?account_id=${encodeURIComponent(accountId)}`
		)
		const data = await response.json() as { roleList: Array<{
			roleName: string
			accountId: string
		}> }
		return data.roleList.map(r => ({
			roleName: r.roleName,
			accountId: r.accountId,
		}))
	}

	async getCredentials(options: GetCredentialsOptions): Promise<SSOCredentials> {
		const params = new URLSearchParams({
			role_name: options.roleName,
			account_id: options.accountId,
		})
		const response = await this.request(`/federation/credentials?${params}`)
		const data = await response.json() as { roleCredentials: {
			accessKeyId: string
			secretAccessKey: string
			sessionToken: string
			expiration: number
		} }
		return {
			accessKeyId: data.roleCredentials.accessKeyId,
			secretAccessKey: data.roleCredentials.secretAccessKey,
			sessionToken: data.roleCredentials.sessionToken,
			expiration: data.roleCredentials.expiration,
		}
	}

	dispose(): void {
		if (this.refreshTimer) {
			clearTimeout(this.refreshTimer)
			this.refreshTimer = null
		}
	}
}
